/**
 * `cadence knowledge` — auto-discovery of local stack reference.
 *
 *   cadence knowledge refresh  → parses docker-compose.yml + .env.example
 *                                 from the repo and writes
 *                                 auto/docs/KNOWLEDGE.md (redacted).
 *
 *   cadence knowledge show     → prints the generated doc.
 *                                 --section <name> filters.
 *
 * Sources and redaction rules are configurable via cadence.config.json's
...
function defaultKnowledgeConfig(): NonNullable<CadenceConfig["knowledge"]> {
  return {
...
function loadConfig(root: string): CadenceConfig | null {
...
export function runKnowledgeShow(options: KnowledgeShowOptions = {}): string {
  ...
  if (!existsSync(path)) {
    throw new Error(
      `Cadence: ${KNOWLEDGE_FILE} not found at ${path}. Run \`cadence knowledge refresh\` first.`,
    );
  }
  ...
  if (section === null) {
    throw new Error(`Cadence: section "${options.section}" not found in ${KNOWLEDGE_FILE}.`);
  }
...
function renderKnowledgeDoc(input: RenderInput): string {
  ...
  lines.push("");
  lines.push(
    "> Auto-generated by `cadence knowledge refresh`. Don't edit by hand — re-run the command.",
  );
  lines.push("");
  ...
  if (input.envVars.length === 0) {
    ...
    lines.push(
      "Values matching the configured redact patterns are masked. Configure under " +
        "`knowledge.redactPatterns` in `cadence.config.json`.",
    );
  }
  lines.push("");
