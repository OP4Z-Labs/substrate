 ...
 * cadence ships zero opinion on which tracker you use; pick one and
 * install its adapter.
 */
...
async function loadOrError(options: TaskCommonOptions): Promise<TaskAdapter> {
  ...
  if (!adapter) {
    throw new Error(
      "Cadence: no task adapter configured.\n" +
        "  Set `extensions.taskAdapter` in cadence.config.json to a package name (e.g.\n" +
        "  `@cadence/adapter-stub` for testing). The reference stub ships at\n" +
        "  packages/adapter-stub/ in the cadence repo.",
    );
  }
  return adapter;
}

function renderTask(task: CadenceTask): string {
  const lines: string[] = [];
...
function emit(options: TaskCommonOptions, payload: CadenceTask | CadenceTask[] | null): void {
...
export async function runTaskFind(options: FindOptions): Promise<CadenceTask | null> {
...
export async function runTaskSearch(options: SearchOptions): Promise<CadenceTask[]> {
  const adapter = await loadOrError(options);
...
export async function runTaskCreate(options: CreateOptions): Promise<CadenceTask> {
  const adapter = await loadOrError(options);
...
export async function runTaskUpdate(options: UpdateOptions): Promise<CadenceTask> {
  const adapter = await loadOrError(options);
  const { cwd: _cwd, json: _json, quiet: _quiet, ...input } = options;
...
export async function runTaskComplete(options: CompleteOptions): Promise<CadenceTask> {
  const adapter = await loadOrError(options);
  const { cwd: _cwd, json: _json, quiet: _quiet, ...input } = options;
