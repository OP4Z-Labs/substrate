...
import type { CadenceConfig, CadenceManifest } from "../src/util/types.js";
...
describe("runInit", () => {
  ...
  it("writes cadence.config.json with project + shortCode + stacks", () => {
    runInit({
    ...
    const config = JSON.parse(readFileSync(configPath, "utf8")) as CadenceConfig;
    ...
    expect(config.version).toBe(CADENCE_VERSION);
  });
  ...
  it("writes an empty manifest stub", () => {
    ...
    expect(manifest.cadenceVersion).toBe(CADENCE_VERSION);
