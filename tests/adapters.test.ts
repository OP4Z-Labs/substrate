 ...
 *
 * These tests deliberately do NOT spawn the cadence CLI — that's the
...
describe("importTaskAdapterModule (stub adapter end-to-end)", () => {
  it("loads the bundled stub adapter and validates the contract", async () => {
    ...
    expect(adapter.name).toBe("@cadence/adapter-stub");
...
describe("built-in git adapter (against a real git repo)", () => {
  ...
  beforeEach(() => {
    ...
    git(["init", "--quiet"], tmp);
    git(["config", "user.email", "test@cadence.dev"], tmp);
    git(["config", "user.name", "Cadence Test"], tmp);
