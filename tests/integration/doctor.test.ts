/**
 * Integration coverage for `cadence doctor`.
 *
 * Smoke step covered (from .agent/SMOKE-2026-05-14.md):
 *
 *   - Step 10 : `cadence doctor` runs all six checks (Node runtime,
...
describe("cadence doctor (integration)", () => {
  let tmp: string;
  ...
  it("smoke 10: doctor on a freshly-initialized repo passes all checks", () => {
    ...
    // wording without coupling to exact counts (cadence version bumps
    ...
    expect(result.stdout).toContain("cadence.config.json");
    expect(result.stdout).toContain("auto/ subdirectories");
    expect(result.stdout).toContain(".cadence-manifest.json");
    ...
    expect(result.stdout.toLowerCase()).toMatch(/marker files|cadence.config/);
  });
  ...
  it("smoke 10: doctor surfaces error severity (exit 1) for a corrupted config", () => {
    ...
    // Corrupt cadence.config.json — invalid JSON.
    writeFileSync(join(tmp, "cadence.config.json"), "{not-valid-json");
