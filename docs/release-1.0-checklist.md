# Substrate v1.0 release checklist
...
- [ ] `packages/substrate/package.json` version bumped to `1.0.0`.
- [ ] `packages/substrate/src/util/version.ts` `SUBSTRATE_VERSION` bumped
      to `1.0.0`.
...
- [ ] `substrate doctor` against a fresh `substrate init` returns all OK.

## Tag

```bash
git tag -a v1.0.0 -m "substrate v1.0 — General Availability"
...
2. Tag: `v1.0.0`
3. Title: `substrate v1.0 — General Availability`
...
```bash
git tag -a v1 -m "@op4z/substrate@v1 (latest)"
...
# If within 72h:
npm unpublish @op4z/substrate@1.0.0
