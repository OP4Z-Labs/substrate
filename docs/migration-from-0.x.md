...
### 1. `cadence audit` runs the detector runtime, not the instruction stub

**v0.x behavior.** `cadence audit --type pre-merge` loaded the
instruction Markdown file and printed a stub message ("the executor
ships in v0.5 / v0.8 / v1.0"). It never ran detectors.

**v1.0 behavior.** `cadence audit` (no `--type`) runs the actual
detector runtime against `cadence/RULES.yaml`. The `--type` legacy
flag still works for the instruction-file surface, but it now
explicitly says "use `cadence audit` to run rules."

**Migration:**

- If you previously relied on `cadence audit --type X` printing a stub,
  update your scripts to either:
  - Use `cadence audit --type X` explicitly (legacy, still works).
  - Use `cadence audit` for the new runtime.
- If you have a `cadence/RULES.yaml`, the new runtime EXECUTES it. Make
...
```bash
cadence audit --strict   # lists every problem in your current RULES.yaml
```

Address each problem before running without `--strict`.

### 4. Audit reports land in `cadence/audits/` (not `auto/audits/`)

**v0.x.** No audit report files (the executor was a stub).

**v1.0.** Reports land at `cadence/audits/<scope>-YYYY-MM-DD.md` plus
`<scope>-latest.json` and `_trend.jsonl`.

**Migration:** Add `cadence/audits/` to `.gitignore` if you don't want
...
### `cadence telemetry show / purge / export`

The opt-in telemetry surface from v0.8 (`cadence config --telemetry
on|off`) is unchanged. v1.0 adds three transparency commands. See
[docs/telemetry-transparency.md](telemetry-transparency.md).

### `cadence uninstall`

New in v1.0. Removes cadence-managed files with a dry-run mode + hash
check to preserve user edits. See `cadence uninstall --help`.
...
Cadence now exports the command functions for programmatic use. See
[docs/programmatic-api.md](programmatic-api.md). The CLI is still the
primary surface; the API is for when you want to embed cadence.
...
during `cadence add standard <name>`).

If you want the new content:

```bash
cadence add standard backend/architecture --overwrite
```

Confirm the prompt; cadence merges your manifest entry.
...
Existing `cadence.config.json` files from v0.x continue to work
...
The GitHub Action (`BeauGoldberg/cadence@v1`) is backward-compatible
with the v0.8 action shape. Inputs and outputs unchanged.

If you pinned `BeauGoldberg/cadence@v0.8`, you can either:
- Stay on `@v0.8` (it continues to work).
- Bump to `@v1` for the new detector runtime.

The action's `version` input (the cadence CLI version) defaults to
...
grep -rn "type: shell" cadence/RULES.yaml auto/RULES.yaml 2>/dev/null

# Quick sanity check on the audit runtime
cadence audit --strict --no-report

# Dry-run the uninstall to see what's tracked
cadence uninstall --dry-run
```
...
If you hit a path that's not covered here, [open a GitHub issue](https://github.com/BeauGoldberg/cadence/issues/new?template=bug_report.yml)
