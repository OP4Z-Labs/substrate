 ...
 *
 * Cadence produces three artefacts per audit run:
 *
 *   1. A human-readable Markdown report at
 *      `cadence/audits/<scope>-YYYY-MM-DD.md`.
 *   2. A `-latest.json` sidecar next to it for tooling consumption
 *      (`/run audit --trend` reads these).
 *   3. An append-only JSONL trend journal at `cadence/audits/_trend.jsonl`.
 *
...
export interface WriteReportOptions {
  /** Absolute repo root. The reports land under `<repoRoot>/cadence/audits/`. */
  repoRoot: string;
...
export function writeAuditReport(
  ...
  const dir = join(options.repoRoot, "cadence", "audits");
...
export function renderMarkdownReport(report: AuditReport): string {
  const lines: string[] = [];
  lines.push(`# Cadence Audit Report — ${report.scope}`);
  lines.push("");
  lines.push(`- **Generated:** ${report.generatedAt}`);
  lines.push(`- **Cadence version:** ${report.cadenceVersion}`);
...
function appendTrendEntry(path: string, report: AuditReport): void {
  ...
  const entry = {
    ...
    scope: report.scope,
    cadenceVersion: report.cadenceVersion,
