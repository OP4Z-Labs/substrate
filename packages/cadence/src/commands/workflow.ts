/**
 * `cadence workflow` — multi-step automation runtime (v0.5).
 *
 ...
 *   - `audit`  : invokes `cadence audit --type <name>` via the
 ...
 *       description: Scaffold a new backend microservice from cadence templates
 *       steps:
...
export function runWorkflowList(options: WorkflowListOptions = {}): WorkflowDefinition[] {
  ...
  if (workflows.length === 0) {
    console.log(
      kleur.yellow("No workflows registered.") +
        "\n" +
        kleur.dim("  Register one with `cadence add workflow <id>`."),
    );
    return workflows;
  }
  ...
  console.log("\n" + kleur.dim("  Describe: cadence workflow describe <id>"));
  console.log(kleur.dim("  Start:    cadence workflow start <id> [--var key=value ...]\n"));
  return workflows;
}
...
export function runWorkflowDescribe(
  ...
  if (!wf) {
    throw new Error(
      `Cadence: workflow "${options.id}" not found. ` +
...
export async function runWorkflowStart(
  ...
  if (wf.steps.length === 0) {
    throw new Error(
      `Cadence: workflow "${options.id}" has no steps defined.\n` +
