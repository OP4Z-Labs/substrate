/**
 * `cadence uninstall` — remove everything cadence wrote to this repo.
 *
 * Discovers:
 *   - `auto/` directory and its contents
 *   - `cadence/` directory (audit reports, RULES.yaml)
 *   - `cadence.config.json` / `cadence.config.yaml`
 *   - Scaffolded standards docs (from `auto/standards/`)
 *   - Bridge files (`.claude/commands/cadence.md`, `.cursor/commands/cadence.md`,
 *     `.cadence/mcp/...`)
 *   - The manifest at `auto/.cadence-manifest.json`
 *
 * Discovery sources:
 *   - The manifest itself (when present and non-empty) lists every file
 *     cadence scaffolded with a content hash.
...
const KNOWN_BRIDGE_PATHS = [
  ".claude/commands/cadence.md",
  ".cursor/commands/cadence.md",
  ".cadence/mcp/cadence-server.json",
  ".cadence/mcp/README.md",
];
...
export function runUninstall(options: UninstallOptions = {}): UninstallResult {
  ...
  // (e.g., `.cadence/`, `.cursor/commands/`).
  for (const rel of [".cadence/mcp", ".cadence", ".cursor/commands", ".claude/commands"]) {
    const abs = join(repoRoot, rel);
...
function renderPlan(plan: UninstallPlanItem[], isDryRun: boolean): void {
  console.log(
    kleur.bold(isDryRun ? "\nCadence uninstall — dry run\n" : "\nCadence uninstall plan\n"),
  );
  if (plan.length === 0) {
    console.log(kleur.dim("  Nothing to remove. (No cadence artefacts found.)"));
    return;
  }
  for (const item of plan) {
