/**
 * `cadence mcp serve` — Model Context Protocol server bridge (v0.8).
 *
 ...
 * the editor reads at startup — the MCP bridge exposes cadence commands
 ...
 * spawns `cadence mcp serve` as a child process and exchanges JSON-RPC
 ...
 * Cadence commands write to process.stdout/stderr directly; for MCP, we
...
export interface McpServeOptions {
  /** Override the cadence working directory the tools resolve against. */
  cwd?: string;
}
...
export function buildMcpServer(options: McpServeOptions = {}): McpServer {
  const server = new McpServer(
    { name: "cadence", version: CADENCE_VERSION },
    {
      capabilities: {
  ...
  server.registerTool(
    "cadence_audit_list",
    {
      ...
      description:
        "Enumerate the audits registered in this cadence project " +
  ...
  server.registerTool(
    ...
    {
      title: "Show cadence knowledge",
      description:
        "Print the auto-discovered KNOWLEDGE.md (or a single section). " +
        "Refreshed by `cadence knowledge refresh` from `docker-compose.yml` " +
        "and `.env.example`.",
      inputSchema: {
        section: z
          .string()
  ...
  server.registerTool(
    "cadence_doctor",
    {
      title: "Diagnose cadence install",
      description:
        "Check installed cadence components (config, manifest, bridges, " +
  ...
  server.registerTool(
    ...
    {
      title: "Check for cadence template drift",
      description:
        "Dry-run of `cadence upgrade --check`. Reports tracked files whose " +
        "templates have changed in this cadence version, without writing " +
        "anything.",
      inputSchema: {},
    },
    async () => {
 ...
 *
 * This is the CLI entry point for `cadence mcp serve`. Tests that need
