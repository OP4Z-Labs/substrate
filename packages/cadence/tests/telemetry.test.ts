 ...
 * directory; the real ~/.config/cadence is never touched.
 */
...
describe("telemetry preference + emission", () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "cadence-telemetry-"));
  ...
  it("setTelemetryEnabled persists the choice to ~/.config/cadence/telemetry.json", () => {
    ...
    expect(pref.cadenceVersion).toBeDefined();
  });
  ...
  it("emitTelemetryEvent writes a line to telemetry.log when enabled", () => {
    ...
    expect(event.cadenceVersion).toBeDefined();
  ...
  it("emitted events never include forbidden fields (paths, identifiers, message bodies)", () => {
    ...
    const allowedKeys = ["v", "ts", "cadenceVersion", "osFamily", "command", "audit", "errorType"];
