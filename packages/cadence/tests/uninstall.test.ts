/**
 * Unit tests for `cadence uninstall`.
 */
...
import { CADENCE_VERSION } from "../src/util/version.js";
...
describe("uninstall", () => {
  ...
  function seedScaffoldedRepo(): void {
    ...
    writeFileSync(join(tmp, "auto", ".cadence-manifest.json"), JSON.stringify(manifest, null, 2), "utf8");

    // Cadence config
    writeFileSync(
      join(tmp, "cadence.config.json"),
  ...
  it("dry-run shows the plan without removing anything", () => {
    ...
    expect(existsSync(join(tmp, "cadence.config.json"))).toBe(true);
  });
  ...
  it("--yes removes tracked + known-location files", () => {
    ...
    expect(existsSync(join(tmp, "cadence.config.json"))).toBe(false);
    expect(existsSync(join(tmp, "cadence"))).toBe(false);
  });
  ...
  it("removes bridge files in known locations", () => {
    ...
    writeFileSync(join(tmp, ".claude", "commands", "cadence.md"), "# bridge\n", "utf8");
    const result = runUninstall({ cwd: tmp, yes: true, quiet: true });
    expect(result.removed).toContain(".claude/commands/cadence.md");
    expect(existsSync(join(tmp, ".claude", "commands", "cadence.md"))).toBe(false);
  });

  it("returns an empty plan when the repo has no cadence artefacts", () => {
    const result = runUninstall({ cwd: tmp, dryRun: true, quiet: true });
