...
describe("audit-runtime: rules loader", () => {
  ...
  it("locateRulesFile prefers cadence/RULES.yaml over auto/RULES.yaml", () => {
    mkdirSync(join(tmp, "cadence"), { recursive: true });
    mkdirSync(join(tmp, "auto"), { recursive: true });
    const cadencePath = join(tmp, "cadence", "RULES.yaml");
    const autoPath = join(tmp, "auto", "RULES.yaml");
    writeFileSync(cadencePath, "rules: []\n", "utf8");
    writeFileSync(autoPath, "rules: []\n", "utf8");
    expect(locateRulesFile(tmp)).toBe(cadencePath);
  });
...
describe("audit-runtime: report writer", () => {
  ...
  it("writes a Markdown report + JSON sidecar + trend journal", async () => {
    ...
    expect(md).toContain("Cadence Audit Report — all");
