...
describe("runDoctor", () => {
  ...
  it("reports an error when cadence.config.json is missing", () => {
  ...
  it("flags stack drift when cadence.config has a stack not detected", () => {
  ...
  it("flags missing Claude bridge file when config enables it", () => {
    ...
    unlinkSync(join(tmp, ".claude", "commands", "cadence.md"));
    const log = vi.spyOn(console, "log").mockImplementation(() => {});
    try {
      const report = runDoctor();
