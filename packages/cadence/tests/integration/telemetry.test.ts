 ...
 *
 * Exercises:
 *   - `cadence config --telemetry on` records enabled=true
 *   - `cadence config --telemetry off` records enabled=false
 *   - `cadence config` (no flag) prints the current preference
 *   - `cadence config --telemetry banana` rejects with exit 2
 *   - After `--telemetry on`, running any command writes one event
 *     line to telemetry.log
 *   - `cadence init` flips the prompted flag so the first-run notice
 *     surfaces exactly once
 *
 * The tests override XDG_CONFIG_HOME so the real ~/.config/cadence is
 * never touched.
 */
...
describe("cadence config --telemetry (integration)", () => {
  let tmp: string;
  let xdgDir: string;

  beforeEach(() => {
    tmp = makeTmpDir();
    xdgDir = mkdtempSync(join(tmpdir(), "cadence-telemetry-xdg-"));
  });
  ...
  it("`cadence config --telemetry on` records enabled=true", () => {
    ...
    const prefPath = join(xdgDir, "cadence", "telemetry.json");
  ...
  it("`cadence config --telemetry off` records enabled=false", () => {
    ...
    const pref = JSON.parse(
      readFileSync(join(xdgDir, "cadence", "telemetry.json"), "utf8"),
    );
    expect(pref.enabled).toBe(false);
  });

  it("`cadence config` (no flag) prints the current preference", () => {
  ...
  it("`cadence config --telemetry banana` rejects with exit 2", () => {
  ...
  it("after --telemetry on, a subsequent command writes its event to telemetry.log", () => {
    ...
    const logFile = join(xdgDir, "cadence", "telemetry.log");
    ...
    expect(events[0].cadenceVersion).toBeDefined();
  ...
  it("`cadence init` surfaces the opt-in notice and flips `prompted` exactly once", () => {
    ...
    expect(first.output).toMatch(/cadence config --telemetry on/);
