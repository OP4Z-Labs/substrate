/**
 * Integration coverage for `cadence mcp serve` — spawning the actual
 ...
 * the response. This is enough to verify the cadence MCP server
...
/**
 * Minimal MCP-over-stdio client. Spawns the cadence binary, performs
    ...
    (async () => {
      try {
        ...
        await send(1, "initialize", {
          ...
          capabilities: { tools: {} },
          clientInfo: { name: "cadence-test-client", version: "0.0.0" },
        });
...
describe("cadence mcp serve (integration)", () => {
  let tmp: string;
