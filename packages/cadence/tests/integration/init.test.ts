/**
 * Integration coverage for `cadence init`.
 *
 * Smoke steps covered (from .agent/SMOKE-2026-05-14.md):
 *
 *   - Step 1  : `cadence init --with-claude` in a fresh tmp dir
 *               generates the auto/ skeleton (7 subdirs),
 *               cadence.config.json, the Claude bridge, the manifest
 ...
 *               `pyproject.toml`). `cadence init` should report
...
describe("cadence init (integration)", () => {
  let tmp: string;
  ...
  it("smoke 1: cadence init --with-claude scaffolds the full v0.1 surface", () => {
    const result = runCli(
    ...
    // cadence.config.json with project + shortCode set.
    const configPath = join(tmp, "cadence.config.json");
    ...
    // The Claude bridge.
    const bridgePath = join(tmp, ".claude", "commands", "cadence.md");
  ...
  it("smoke 1: no stack flag + no marker files → fallback default (python + typescript)", () => {
    ...
    const config = JSON.parse(readFileSync(join(tmp, "cadence.config.json"), "utf8"));
    expect(config.stacks).toEqual(["python", "typescript"]);
  });

  it("smoke 11: cadence init auto-detects python from bare pyproject.toml", () => {
