/**
 * Integration coverage for `cadence audit`.
 *
 * Smoke steps covered (from .agent/SMOKE-2026-05-14.md):
 *
 *   - Step 2 : `cadence audit --list` enumerates the scaffolded audits
 ...
 *              (catalog)" section.
 *
 *   - Step 3 : `cadence audit --type pre-merge` returns a structured
 ...
 *              e504ce2).
 *
 *   - Step 4 : `cadence audit --type backend` against a fresh init
...
describe("cadence audit (integration)", () => {
  let tmp: string;
  ...
  it("smoke 2: audit --list enumerates scaffolded audits + bundled catalog", () => {
    ...
    expect(result.stdout).toContain("cadence audit --type");
    expect(result.stdout).toContain("cadence add audit");
  });

  it("smoke 2: audit --list --json emits the structured envelope", () => {
    // The JSON shape is a downstream contract for any cadence-aware
  ...
  it("smoke 4: audit --type backend fails with actionable error when not scaffolded", () => {
    ...
    // has to `cadence add audit backend` first. The integration test
