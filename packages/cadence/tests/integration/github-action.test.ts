/**
 * Integration coverage for the cadence GitHub Action (v0.8).
 *
 * Validates:
 ...
 *
 * The action shells out to `npx cadence@<version>` for the real run,
 * which would hit npm. To keep CI offline-friendly, the action test
 * here overrides PATH so a local-built cadence shim is reached first;
 ...
 * `.github/workflows/cadence-action-test.yml` dogfood workflow.
 */
...
describe("cadence GitHub Action", () => {
  let tmp: string;
  ...
  it("action exits 0 + writes report when the wrapped cadence command succeeds", () => {
    // Stub `npx` on PATH so the action's `npx -y cadence@... <args>`
    ...
    const reportPath = join(workdir, "cadence-report.json");
  ...
  it("action exits non-zero when wrapped cadence command fails (fail-on=error)", () => {
    const shimDir = join(tmp, "shim");
    mkdirSync(shimDir, { recursive: true });
    const shimPath = join(shimDir, "npx");
    writeFileSync(
  ...
  it("action exits 0 when fail-on=none, regardless of underlying cadence exit code", () => {
    const shimDir = join(tmp, "shim");
    mkdirSync(shimDir, { recursive: true });
    const shimPath = join(shimDir, "npx");
    writeFileSync(
