/**
 * Unit tests for `substrate upgrade` — programmatic API.
 *
...
describe("planUpgrade", () => {
  ...
  it("classifies ejected files when the manifest flag is true", () => {
    runAdd({ category: "audit", item: "backend", cwd: tmp, quiet: true });
    // Manually set ejected=true to simulate `substrate config --eject`.
    ...
    target!.ejected = true;
    writeFileSync(
      join(autoDir, ".substrate-manifest.json"),
...
describe("runUpgrade --apply", () => {
  ...
  it("writes a .substrate-merge sidecar when choice is `merge`", async () => {
    runAdd({ category: "audit", item: "backend", cwd: tmp, quiet: true });
    const filePath = join(tmp, "auto", "instructions", "main", "audit-backend.md");
    const userContent = "# my custom audit\n";
    writeFileSync(filePath, userContent);
    ...
    expect(readFileSync(filePath, "utf8")).toBe(userContent);
    const mergePath = filePath + ".substrate-merge";
    ...
    expect(applied?.mergePath).toContain(".substrate-merge");
  });
 ...
 * and the merge UX emits a richer .substrate-merge file.
 *
 * Templates-history for v0.5.0 ships in this build because v0.5 is the
 * version substrate@0.8.0 first introduced history for. The `add` call here
...
describe("runUpgrade --apply (v0.8 three-way merge)", () => {
  ...
  it("emits a three-way conflict-marker block in .substrate-merge when threeWay is true", async () => {
    runAdd({ category: "audit", item: "backend", cwd: tmp, quiet: true });
    rewriteManifestVersion(join(tmp, "auto"), "audit-backend.md", "0.5.0");
