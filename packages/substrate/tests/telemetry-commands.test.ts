/**
 * Unit tests for `substrate telemetry show / purge / export`.
 *
...
describe("telemetry commands", () => {
  let tmpHome: string;
  beforeEach(() => {
    tmpHome = makeTempDir("substrate-tel-home-");
  ...
  function seedLog(events: Array<Record<string, unknown>>): string {
    const dir = join(tmpHome, "substrate");
  ...
  it("show returns the preference + parsed events from the log", () => {
    ...
    seedLog([
      { v: 1, ts: "2026-05-14T01:00:00Z", substrateVersion: "1.0.0", osFamily: "linux", command: "audit" },
      { v: 1, ts: "2026-05-14T01:00:01Z", substrateVersion: "1.0.0", osFamily: "linux", command: "init" },
    ]);
  ...
  it("purge without --yes is a no-op (with hint)", () => {
    ...
    expect(existsSync(join(tmpHome, "substrate", "telemetry.json"))).toBe(true);
    expect(existsSync(join(tmpHome, "substrate", "telemetry.log"))).toBe(true);
  });
  ...
  it("purge --yes removes both files", () => {
    ...
    expect(existsSync(join(tmpHome, "substrate", "telemetry.json"))).toBe(false);
    expect(existsSync(join(tmpHome, "substrate", "telemetry.log"))).toBe(false);
  });
  ...
  it("export writes CSV with header when format=csv", () => {
    setTelemetryEnabled(true);
    seedLog([
      { v: 1, ts: "2026-01-01T00:00:00Z", substrateVersion: "1.0.0", osFamily: "linux", command: "audit" },
    ]);
    const out = join(tmpHome, "export.csv");
    runTelemetryExport({ outPath: out, format: "csv", quiet: true });
    const text = readFileSync(out, "utf8");
    expect(text).toContain("v,ts,substrateVersion,osFamily,command,audit,errorType");
    expect(text).toContain("audit");
  });

  it("export quotes cells containing commas", () => {
    seedLog([{ command: "a,b,c", substrateVersion: "1.0.0" }]);
  ...
  it("show with no log returns empty events without throwing", () => {
    // No log file at all.
    writeFileSync(join(tmpHome, "substrate-marker"), "x", "utf8"); // ensure dir exists for other reason
    const result = runTelemetryShow({ quiet: true });
