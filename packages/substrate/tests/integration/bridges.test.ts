 ...
 * the v0.5 contract:
 *
 *   - --bridge claude scaffolds .claude/commands/substrate.md
 *   - --bridge cursor scaffolds .cursor/commands/substrate.md
 *   - --bridge claude,cursor scaffolds both, neither one wiping the other
 *   - --with-claude (legacy) still works as an alias
 *   - Both bridges are independently configurable in substrate.config.json
...
describe("substrate bridges (integration)", () => {
  let tmp: string;
  ...
  it("--bridge claude scaffolds the claude bridge only", () => {
    ...
    expect(existsSync(join(tmp, ".claude", "commands", "substrate.md"))).toBe(true);
    expect(existsSync(join(tmp, ".cursor", "commands", "substrate.md"))).toBe(false);
  ...
  it("--bridge cursor scaffolds the cursor bridge only", () => {
    ...
    const cursorBridgePath = join(tmp, ".cursor", "commands", "substrate.md");
    ...
    expect(bridgeContent).toContain("/substrate");
    expect(bridgeContent).toContain("npx @op4z/substrate");
  ...
  it("--bridge claude,cursor scaffolds both bridges in one invocation", () => {
    ...
    expect(existsSync(join(tmp, ".claude", "commands", "substrate.md"))).toBe(true);
    expect(existsSync(join(tmp, ".cursor", "commands", "substrate.md"))).toBe(true);
  ...
  it("doctor flags a missing bridge file as an error", () => {
    ...
    // surface as an error.
    unlinkSync(join(tmp, ".cursor", "commands", "substrate.md"));
  ...
  // markdown slash-command bridges (substrate-server.json + README.md).
  it("--bridge mcp scaffolds the MCP server registration + README", () => {
    ...
    const serverPath = join(tmp, ".substrate", "mcp", "substrate-server.json");
    const readmePath = join(tmp, ".substrate", "mcp", "README.md");
    expect(existsSync(serverPath)).toBe(true);
    expect(existsSync(readmePath)).toBe(true);

    // Server JSON shape: top-level mcpServers, keyed by `substrate-<SHORT_CODE>`.
    const registration = JSON.parse(readFileSync(serverPath, "utf8"));
    expect(registration.mcpServers).toBeDefined();
    expect(Object.keys(registration.mcpServers)).toContain("substrate-MC");
    const entry = registration.mcpServers["substrate-MC"];
    expect(entry.command).toBe("npx");
    expect(entry.args).toEqual(["substrate", "mcp", "serve"]);
  ...
  it("--bridge claude,cursor,mcp scaffolds all three bridges", () => {
    ...
    expect(existsSync(join(tmp, ".claude", "commands", "substrate.md"))).toBe(true);
    expect(existsSync(join(tmp, ".cursor", "commands", "substrate.md"))).toBe(true);
    expect(existsSync(join(tmp, ".substrate", "mcp", "substrate-server.json"))).toBe(true);
  ...
  it("doctor flags missing mcp server json as an error", () => {
    ...
    unlinkSync(join(tmp, ".substrate", "mcp", "substrate-server.json"));
