 ...
 *   - The home page contains the substrate brand string
 ...
 *   SUBSTRATE_SKIP_DOCS_BUILD=1 npm test
 */
...
const SKIP = process.env.SUBSTRATE_SKIP_DOCS_BUILD === "1";
...
describe.skipIf(SKIP)("docs-site (Astro static build)", () => {
  it(
    ...
    () => {
      ...
      // Home contains the substrate brand line.
      const home = readFileSync(join(DIST, "index.html"), "utf8");
      expect(home).toContain("Substrate");
  ...
  it.skipIf(SKIP)(
    "docs-site is dogfood-scaffolded with substrate (substrate.config.json present)",
    () => {
      const configPath = join(DOCS_SITE, "substrate.config.json");
      expect(existsSync(configPath), "docs-site must be substrate-init'd").toBe(true);
      const config = JSON.parse(readFileSync(configPath, "utf8"));
      expect(config.project.name).toBe("substrate-docs-site");
    },
  );
});
