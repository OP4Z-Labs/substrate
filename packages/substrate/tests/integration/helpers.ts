/**
 * Integration-test harness for the `substrate` CLI binary.
 *
 ...
 *    `3995a60`): 84 unit tests passed cleanly while `substrate --help`
 ...
 *    No need for async I/O — every substrate command is a one-shot.
...
export const SUBSTRATE_REPO_ROOT = REPO_ROOT;
...
/**
 * Spawn the built substrate CLI with the given argv.
 *
...
export function runCli(args: string[], options: CliRunOptions): CliRunResult {
  ...
  if (result.error) {
    throw new Error(
      `Failed to spawn substrate CLI: ${result.error.message}. ` +
 ...
 *
 * Uses a `substrate-int-` prefix so collateral on the OS tmpdir is
 * trivial to identify and purge. Callers should pair every `makeTmpDir`
 * with a `removeTmpDir` in `afterEach`.
 */
export function makeTmpDir(): string {
  return mkdtempSync(join(tmpdir(), "substrate-int-"));
}
