/**
 * Integration coverage for `substrate add` subcommands.
 *
 * Smoke steps covered (from .agent/SMOKE-2026-05-14.md):
 *
 *   - Step 5  : `substrate add audit security` writes audit-security.md
 ...
 *
 *   - Step 6  : `substrate add standard backend/architecture` writes
 ...
 *
 *   - Step 7  : `substrate add scaffold package-ts` appends an entry to
 *               auto/config/scaffolds.yaml. As designed, no template
 *               body is copied — `substrate create` pulls from
 *               node_modules at create-time.
 *
 *   - Step 12 : Idempotency. Re-running `substrate add audit security`
...
function readManifest(tmp: string): Manifest {
  const path = join(tmp, "auto", ".substrate-manifest.json");
  return JSON.parse(readFileSync(path, "utf8")) as Manifest;
}

describe("substrate add (integration)", () => {
  let tmp: string;
  ...
  it("smoke 5: substrate add audit security scaffolds + records sha256 in manifest", () => {
    ...
    // record a sha256-prefixed hash, the substrate version that wrote it,
  ...
  it("smoke 6: substrate add standard backend/architecture writes the doc + manifest entry", () => {
    const result = runCli(
  ...
  it("smoke 6: substrate add standard rejects the . delimiter with actionable error", () => {
  ...
  it("smoke 7: substrate add scaffold package-ts registers in scaffolds.yaml", () => {
  ...
  it("smoke 12: re-running substrate add audit security preserves the existing file", () => {
