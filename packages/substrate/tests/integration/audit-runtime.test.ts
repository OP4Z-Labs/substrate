/**
 * Integration tests for `substrate audit` (v1.0 detector runtime).
 *
 * Spawn the built CLI against a fresh tmp dir with a hand-rolled
 * `substrate/RULES.yaml`. Asserts exit code, output shape, and the report
 * artefacts on disk.
 */
...
describe("integration: substrate audit (detector runtime)", () => {
  let tmp: string;
  beforeEach(() => {
    tmp = makeTmpDir();
  });
  ...
  function writeRules(yaml: string): void {
    ...
    writeFileSync(join(tmp, "substrate", "RULES.yaml"), yaml, "utf8");
  }
  ...
  it("runs all rules and emits a Markdown + JSON report", () => {
    ...
    // Report files should land under substrate/audits/.
  ...
  it("--json outputs the full report envelope on stdout", () => {
    ...
    expect(existsSync(join(tmp, "substrate", "audits"))).toBe(false);
  });
});
