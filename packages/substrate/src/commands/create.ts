...
function validateName(name: string): void {
  if (!/^[a-z][a-z0-9-]*$/.test(name)) {
    throw new Error(
      `Substrate: invalid name "${name}". Use lowercase kebab-case (e.g. my-package).`,
    );
  }
}
...
export function runCreate(options: CreateOptions): CreateResult {
  ...
  if (!options.template) {
    throw new Error(`Substrate: --template is required. ${availableTemplatesHint(templatesRoot)}`);
  }
  if (!options.name) {
  ...
  if (!existsSync(templateDir) || !statSync(templateDir).isDirectory()) {
    throw new Error(
      `Substrate: template "${options.template}" not found. ${availableTemplatesHint(templatesRoot)}`,
    );
  }
  if (!isScaffoldableTemplate(options.template)) {
    throw new Error(
      `Substrate: template "${options.template}" is not a scaffold template (reserved for internal use).`,
    );
  }
  ...
  if (existsSync(destination)) {
    ...
    if (entries.length > 0) {
      throw new Error(
        `Substrate: destination ${destination} already exists and is non-empty. Aborting to avoid clobbering.`,
      );
    }
  }
...
function isScaffoldableTemplate(name: string): boolean {
  ...
  // they're consumed by `substrate init`, not exposed to `substrate create`.
