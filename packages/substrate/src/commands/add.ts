/**
 * `substrate add <item>` — shadcn-parallel single-item scaffolding.
 *
 ...
 *
 * Every successful add updates `auto/.substrate-manifest.json` so v0.5's
...
export function runAdd(options: AddOptions): AddResult {
  ...
  if (!existsSync(autoDir)) {
    throw new Error(
      `Substrate: auto/ directory not found at ${autoDir}. Run \`substrate init\` first.`,
    );
  }
  ...
  switch (options.category) {
    ...
    default: {
      ...
      throw new Error(`Substrate: unknown add category "${exhaustive as string}"`);
    }
  }
}
...
function addAudit(root: string, autoDir: string, opts: AddOptions): AddResult {
  ...
  if (!existsSync(sourcePath)) {
    ...
    throw new Error(
      `Substrate: audit template "${opts.item}" not found. ` +
...
function addStandard(root: string, autoDir: string, opts: AddOptions): AddResult {
  ...
  if (parts.length !== 2) {
    throw new Error(
      `Substrate: standard item must be "<scope>/<area>" (e.g. "backend/architecture"). Got "${opts.item}".`,
    );
  }
  ...
  if (!sourcePath) {
    ...
    throw new Error(
      `Substrate: standard "${opts.item}" not found. Available:\n  ${available.join("\n  ")}`,
    );
  }
...
function addScaffold(root: string, autoDir: string, opts: AddOptions): AddResult {
  ...
  // framework package and is consumed by `substrate create`. We just record
  // it in auto/config/scaffolds.yaml so `substrate doctor` can verify the
  ...
  if (!existsSync(templateDir)) {
    ...
    throw new Error(
      `Substrate: scaffold template "${opts.item}" not found. ` +
        `Available: ${available.join(", ")}`,
    );
  }
...
function writeScaffoldsRegistry(path: string, registry: ScaffoldsRegistry): void {
  const lines = [
    "# Scaffold registry — managed by `substrate add scaffold <name>`.",
    "# Entries here are templates available to `substrate create --template <name>`.",
    "",
    "scaffolds:",
...
> Substrate scaffold — fill in the TODOs.
...
function writeWorkflowsRegistry(path: string, registry: WorkflowsRegistry): void {
  const lines = [
    "# Workflow registry — managed by `substrate add workflow <id>`.",
