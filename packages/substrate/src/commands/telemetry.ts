/**
 * `substrate telemetry` subcommands — transparency surfaces.
 *
 * v1.0 adds three commands beyond the v0.8 `config --telemetry on|off`:
 *
 *   - `substrate telemetry show`    : dump the current preference + log
 *                                   tail.
 *   - `substrate telemetry purge`   : wipe preference + log (with prompt).
 *   - `substrate telemetry export`  : copy the log to a user-specified
 *                                   file (CSV or JSONL).
 *
...
export function runTelemetryShow(options: TelemetryShowOptions = {}): {
  ...
  console.log(kleur.bold("Substrate telemetry status"));
  console.log(
    `  state    : ${
      pref.enabled === true
        ? kleur.green("enabled")
  ...
  for (const e of recent) {
    const o = e as { ts?: string; command?: string; substrateVersion?: string; osFamily?: string; errorType?: string };
    console.log(
      `  ${o.ts ?? "?"}  ${kleur.cyan((o.command ?? "?").padEnd(12))}  ${kleur.dim(`v${o.substrateVersion ?? "?"} ${o.osFamily ?? "?"}`)}${o.errorType ? kleur.red(`  err=${o.errorType}`) : ""}`,
    );
  }
  return { preference: pref, events };
}
...
export function runTelemetryPurge(options: TelemetryPurgeOptions = {}): {
  ...
  if (!options.yes && !options.json) {
    console.log(
      kleur.yellow("This will delete substrate's local telemetry preference and event log."),
    );
...
export function runTelemetryExport(options: TelemetryExportOptions): {
  ...
  let payload: string;
  if (format === "csv") {
    const cols = ["v", "ts", "substrateVersion", "osFamily", "command", "audit", "errorType"];
