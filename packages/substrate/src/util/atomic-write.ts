 ...
 *
 * Why this exists: every substrate write that lands user-visible content
 ...
 *     the trade-off here — substrate output is regeneratable from
 *     templates, and the additional syscall would gate every write on a
 *     full directory flush.
 *
 * The tmp filename pattern is `<basename>.substrate-tmp-<random>` so a
 * crash-orphan is identifiable; nothing else in substrate creates files
 * with this suffix.
 */
...
const TMP_SUFFIX = ".substrate-tmp-";
