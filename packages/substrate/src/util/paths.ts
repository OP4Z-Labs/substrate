 ...
 *
 * Why not `require.resolve("substrate/templates")`? That requires
 * `substrate` to be installed (or a self-reference declared in
...
export function getTemplatesDir(): string {
  ...
  throw new Error(
    `Substrate: could not locate bundled templates directory (started from ${HERE}). ` +
 ...
 * support the real three-way merge in `substrate upgrade --apply`.
 *
 * Layout:
 *   templates-history/
 *     0.5.0/                  ← snapshot of `templates/` shipped with substrate@0.5.0
 *     0.8.0/                  ← snapshot of `templates/` shipped with substrate@0.8.0
 *     ...                     ← one per shipped version going forward
 *
 * The directory is OPTIONAL — if a substrate build lacks it (e.g. someone is
 ...
 * the root. v0.3's `substrate doctor` may add upward search once we have
 * `substrate.config.json` to anchor on, but commands are explicit for now.
 */
...
/**
 * The seven directories that `substrate init` creates under `auto/`.
 *
