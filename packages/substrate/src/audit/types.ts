 ...
 *
 * The runtime executes rules loaded from `substrate/RULES.yaml` (or whatever
 * path `substrate.config` points at) and produces a structured
 * {@link AuditReport}. Both the CLI (`substrate audit`) and the
...
export interface ScriptDetector {
  type: "script";
  /**
   * Repo-relative path to the script (e.g. `substrate/detectors/no-large-files.js`).
   ...
   * must be compiled by the consumer first (substrate does not bundle a TS
  ...
  export?: string;
  /**
   * Arbitrary options the rule author wants to pass to the script. Substrate
...
export interface AuditReport {
  ...
  schemaVersion: 1;
  /** Substrate version that produced the report. */
