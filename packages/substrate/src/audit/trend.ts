 ...
 *
 * Reads the append-only `substrate/audits/_trend.jsonl` and returns the
 * history of audit runs grouped by scope. Used by `substrate audit --trend`.
 */
...
export function readTrend(repoRoot: string): TrendSummary {
  const path = join(repoRoot, "substrate", "audits", "_trend.jsonl");
  if (!existsSync(path)) {
