 ...
 *
 * Note on TS: substrate does NOT transpile script paths at runtime —
...
function packageRootFromHere(): string {
  // Walk up at most 5 levels looking for a `package.json` with name "@op4z/substrate".
  ...
  for (let depth = 0; depth < 6; depth += 1) {
    ...
    if (existsSync(pkgPath)) {
      try {
        ...
        if (pkg && pkg.name === "@op4z/substrate") return cursor;
      } catch {
        // ignore — keep walking
      }
    }
...
export async function runScriptDetector(
  ...
  if (!existsSync(harness)) {
    throw new Error(
      `script worker harness not found at ${harness}. This usually means substrate was not built.`,
    );
  }
