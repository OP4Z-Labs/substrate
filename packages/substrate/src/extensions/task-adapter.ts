 ...
 *
 * Substrate keeps its task-management surface neutral: a thin set of verbs
 ...
 * default — `substrate task` commands then emit a "no adapter configured"
 ...
 *
 * The reference stub adapter (`@op4z/substrate-adapter-stub`, shipped in this
 * monorepo at `packages/adapter-stub/`) implements every method as a
 * "would call <verb>" log line, proving the plugin contract works
 * end-to-end without binding substrate to any specific tracker SDK.
 */
...
export interface SubstrateTask {
  ...
  description?: string;
  /**
   * Domain-shaped status string. Substrate does not enumerate the valid
...
/**
 * The minimal verb surface substrate relies on. Adapters return promises
 * uniformly — even when the underlying call is synchronous — so the
 * caller doesn't need to special-case.
 */
export interface TaskAdapter {
  /** Display name, surfaced in `substrate doctor` and CLI banners. */
  readonly name: string;
  /** Semver string of the adapter (informational; not enforced by substrate). */
