...
> **Substrate default standard.** What runs on every PR, what runs on
...
5. **Pre-merge audit** — `substrate audit --diff` (rule `INF-CICD-001`).
...
jobs:
  ...
  audit:
    needs: lint
    steps:
      - run: npx @op4z/substrate audit --diff --fail-on error
```
